...
def test_different_delimiter():
    """join using different delimiter -> ;"""
    delimiters = ["-", "#"]
    for delimiter in delimiters:
        arg = "bananas apples dates cherries"
        out = getoutput(f"{prg} {arg} --delimiter=" + delimiter)
        expected = f"You are bringing bananas{delimiter} apples{delimiter} dates{delimiter} and cherries."
        assert out.strip() == expected

def test_no_oxford_comma():
    """join using no oxford comma flag"""
    arg = "bananas apples dates cherries"
    out = getoutput(f"{prg} {arg} --no-oxford-comma")
    expected = f"You are bringing bananas, apples, dates and cherries."
    assert out.strip() == expected
